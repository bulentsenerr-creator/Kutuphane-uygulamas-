/*
   ...
   - Çoklu CDN + Lokal (opsiyonel) fallback
   - Android Chrome getUserMedia constraints güçlendirildi
   - Hata teşhisi: scanInfo alanına err.name/mesaj eklenir
*/
...
function cameraErrorMessage(err){
  ...
  if(/Content Security Policy|CSP|Refused to load the script|violates the following Content Security Policy/i.test(msg)){
    return 'ZXing yüklenemedi: Site güvenlik politikası (CSP) dış scriptleri engelliyor olabilir. Çözüm: ZXing dosyasını aynı domain altında barındırın veya CSP\'de script-src izin verin.';
  }

  if (!window.isSecureContext) {
    return 'Kamera için HTTPS gereklidir (güvenli bağlantı). Dosyayı https bir adresten açın.';
  }
  if (name === 'NotAllowedError' || name === 'SecurityError') {
    return 'Kamera izni reddedildi. Tarayıcı ayarlarından kamera iznini açın.';
  }
  if (name === 'NotFoundError' || name === 'DevicesNotFoundError') {
    return 'Kamera bulunamadı. Cihazınızda kamera yok veya erişilemiyor.';
  }
  if (name === 'NotReadableError' || name === 'TrackStartError') {
    return 'Kamera şu anda başka bir uygulama tarafından kullanılıyor olabilir. Diğer uygulamaları kapatıp tekrar deneyin.';
  }
  if (name === 'OverconstrainedError' || name === 'ConstraintNotSatisfiedError') {
    return 'Seçilen kamera/ayarlar desteklenmiyor. Farklı kamera seçip tekrar deneyin.';
  }
  return 'Kamera başlatılamadı. İzinleri ve bağlantıyı kontrol edip tekrar deneyin.';
}
...
async function openScanner(){
  ...
  try{
    ...
    if(m.includes('ZXing yüklenemedi')){
      $('scanInfo').textContent = 'Barkod kütüphanesi (ZXing) yüklenemedi. Büyük olasılıkla CDN engeli / CSP / reklam engelleyici var. (Console\'a bakın)';
    } else {
      $('scanInfo').textContent = cameraErrorMessage(err);
    }
    console.error(err);
  }
}
...
function closeScanner(e){
  if(e && e.target && e.target.id==='scanModal'){
    hideScanner();
  }
}
...
async function loadZXing(){
  ...
  const sources = [
    ...
    'https://unpkg.com/@zxing/library@0.18.3-dev.7656630/umd/index.js'
  ];
  ...
  const loadScript = (src, timeoutMs=15000)=> new Promise((resolve, reject)=>{
    ...
    s.onload = ()=>{
      setTimeout(()=>{
        clearTimeout(timer);
        if(window.ZXing && (ZXing.BrowserMultiFormatReader || ZXing.BrowserBarcodeReader)){
          done = true; resolve(true);
        } else {
          try{ s.remove(); }catch(e){}
          reject(new Error('Yüklendi ama ZXing global oluşmadı: '+src));
        }
      }, 0);
    };
  ...
  for(let i=0;i<sources.length;i++){
    ...
    try{
      ...
      console.warn('ZXing load fail:', sources[i], e);
    }
  }
...
async function listCameras(){
  try{
    if(!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) throw new Error('enumerateDevices yok');
...
async function startScan(){
  try{
    ...
    try{
      ...
      const n1 = err1?.name || '';
      if (n1 === 'OverconstrainedError' || n1 === 'ConstraintNotSatisfiedError'){
        try{
    ...
    const extra = ` (${e?.name || 'Error'}: ${(e?.message || '').toString().slice(0,120)})`;
    $('scanInfo').textContent = cameraErrorMessage(e) + extra;
    console.error(e);
  }
}
...
function stopScan(){
  ...
  try{ const video = $('preview'); if(video){ video.pause?.(); video.srcObject = null; } }catch(e){}
  trackRef = null;
  streamRef = null;
}
...
async function toggleTorch(){
  try{
    ...
    console.warn('Torch desteklenmiyor');
  }
}
...
function extractIsbn(txt){
  ...
  try{
    const u = new URL(s);
    const q = u.searchParams.get('isbn') || u.searchParams.get('ISBN');
...
async function scanFromImage(e){
  ...
  img.onload = async ()=>{
    try{
      ...
      if(res && res.getText){
        onScanResult(res.getText());
      } else {
        $('scanInfo').textContent = 'Barkod bulunamadı';
        beep(440, 120);
      }
    }catch(err){
      $('scanInfo').textContent = 'Çözümlenemedi';
      console.error(err);
    }
  };
